...
use super::{ComponentMap, LinkDescription, MarkdownMouseEvent, MdComponentProps};
...
fn is_probably_custom_component(raw_html: &str) -> bool {
    raw_html.chars().filter(|x| x == &'<' || x == &'>').count() == 2
}
    ...
    I: Iterator<Item = (Event<'a>, Range<usize>)>,
{
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let rendered = match item {
            ...
            pulldown_cmark_wikilink::Event::InlineHtml(_) => todo!(),
        };
    ...
    I: Iterator<Item = (Event<'a>, Range<usize>)>,
{
    ...
    fn html(&mut self, s: &str, range: Range<usize>) -> Option<Result<View, HtmlError>> {
        match (&self.current_component, self.end_tag) {
            (None, None) if is_probably_custom_component(s) => Some(self.custom_component(s)),
            (None, _) => {
            ...
            (Some(x), None) if is_probably_custom_component(s) => Some(HtmlError::err(&format!(
                "the component `{x}` is not properly closed"
            ))),
            (Some(x), Some(_)) if s.trim() == format!("</{x}>") => Some(HtmlError::err(&format!(
    ...
    fn render_tag(&mut self, tag: Tag<'a>, range: Range<usize>) -> Result<View, HtmlError> {
        Ok(match tag.clone() {
            ...
            .into_view(),
            Tag::List(None) => view! {<ul>{self.children(tag)}</ul>}.into_view(),
            ...
            pulldown_cmark_wikilink::Tag::HtmlBlock => todo!(),
        })
    }
}

fn render_tasklist_marker(context: &RenderContext, m: bool, position: Range<usize>) -> View {
    let callback = context.onclick.clone();
...
fn render_code(context: &RenderContext, s: &str, range: Range<usize>) -> View {
    let callback = make_callback(context, range);
    view! {<code on:click=callback>{s.to_string()}</code>}.into_view()
}
...
fn render_link(context: &RenderContext, link: LinkDescription) -> Result<View, HtmlError> {
